...
in project's parent config file or using system property which are required to enable Smart Testing.
...
<2> inside this file we have `strategies: new` & `scm: lastChanges: 1` defined.(You can set it using respective system property also).
