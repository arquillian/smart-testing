...
All reports from previous local build are automatically copied by the maven extension to a temp directory `${project.directory}/.smart-testing/execution/reports` and when the build is finished the directory is removed.
